...
const ciphers = ["TLS_CHACHA20_POLY1305_SHA256", "TLS_AES_128_GCM_SHA256", "TLS_AES_256_GCM_SHA384", "TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256"];
...
export async function credentialsAuth(username: string, password: string) {
  ...
  if (!username || !password) return console.log("No valorant auth username or password");
  ...
  console.log(tokens.access_token.split(".")[1]);
  tokens.subject = JSON.parse(Buffer.from(tokens.access_token.split(".")[1], "base64").toString()).sub;
  return tokens;
}
...
export async function lockfileAuth() {
  ...
  const contents = await fs.promises.readFile(path.join(process.env.LOCALAPPDATA!, "Riot Games\\Riot Client\\Config\\lockfile"), "utf8");
  let lD: any = {};
  ...
  let entitlements = await axiosGet(
    ...
    {
      Authorization: "Basic " + Buffer.from(`riot:${lD.password}`, "utf8").toString("base64"),
      "X-Riot-ClientVersion": version
    },
    null
  ).catch(function (err) {
